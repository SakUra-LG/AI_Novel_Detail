...
def static_audit(main, title: str, text: str) -> dict:
    ...
    checks = {
        ...
        "humor_marker_min": humor_marker_count >= 14,
    ...
    messages = [
        {
            ...
            "content": (
                ...
                "从故事完整性、幽默强度、阿满十八篇串联功能、乱码/污染、字数体感五方面打分。"
            ),
        },
        {
            "role": "user",
...
def generate_story(title: str, out_path: Path, log_path: Path, api_timeout: int, api_retries: int, api_transport: str, process_timeout: int) -> dict:
    prompt = f"改写神话故事{title}，要求有幽默，正文目标8000字"
    code = (
        "import sys; "
        f"sys.path.insert(0, {str(CURRENT_DIR)!r}); "
        "from main import generate_myth_rewrite; "
        "title=sys.argv[1]; out=sys.argv[2]; "
        "text=generate_myth_rewrite(f'改写神话故事{title}，要求有幽默，正文目标8000字'); "
    ...
    with log_path.open("w", encoding="utf-8") as log:
        ...
        try:
            completed = subprocess.run(
                [sys.executable, "-X", "utf8", "-c", code, title, str(out_path)],
            ...
            return {
                "generated": completed.returncode == 0 and out_path.exists(),
...
def main_cli():
    parser = argparse.ArgumentParser(description="批量生成并审计18篇神话改写质量。")
    parser.add_argument("--stories", nargs="*", default=None, help="只跑指定故事名；默认跑18篇。")
