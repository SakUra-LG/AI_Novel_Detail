...
def get_myth_system_prompt_base(reference_content=None):
    ...
    return f"""
            ...
            - 【对话是第一幽默载体（必须严格遵守）】：
              ...
              - 每个节拍卡对应的小节中，【至少 2 个笑点】，且【至少 1 个必须是对话笑点】（甲说—乙接，或甲吐槽—乙拆台/接梗）。非关键庄重节拍可要求【至少 2 个对话笑点】。
            ...
            - 【必须参考样本集】：必须参考知识库中"神话重写·哪吒风格（幽默+亲子）"的样本，学习其对话节奏、拆台方式、夸张与反差的用法。
            - 【禁止内容】：现代职场/网络流行语（打工人、内卷、绝了等）、低俗/身体羞辱/虐人取乐、破坏神话世界观的设定。其余以「好笑、对白多、类型多样」为准。
...
请将以上总体大纲分配到三幕，按【背景→高潮过程→结果】划分，生成三幕分别的【详细】大纲和【镜头节拍卡】。要求：
...
4. 第一幕 4-5 张节拍卡，第二幕 6-8 张节拍卡，第三幕 4-5 张节拍卡。每张节拍卡必须包含以下字段：
   - 场景目标：这一小段要完成什么叙事功能
   - 画面要素：至少2个可拍摄的画面/动作细节（非抽象词）
   - 情绪推动：角色此刻情绪从A到B
   - 信息增量：这一节新增的信息是什么（不能重复前文）
...
4. 幽默强度与通道要求（本改写以幽默为先，宁可笑点明显也不要为文学性牺牲好笑）：
   ...
   - 若当前节拍卡的"场景目标"属于铺垫、日常、情绪累积、尝试行动、寻找突破口、行动间隙等【非关键情节】：本小节【必须出现至少 2 个笑点】，其中【至少 1 个必须是对白笑点】（甲说—乙接，或互怼、拆台、调侃）；另一个可继续对白或旁白/动作反差。类型上须在本小节内用到【夸张、反差、错误逻辑、嘴硬、自嘲、拆台】中的至少 1 种，且鼓励多用对白呈现。
   ...
   - 单次笑点 1-2 句话，允许【连续 2 句「一抛一接」】对白；严禁大段独白无接话。严禁现代职场/网络流行语、低俗/侮辱性桥段。
