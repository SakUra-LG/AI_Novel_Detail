...
def get_myth_system_prompt_base(reference_content=None):
    ...
    return f"""
            ...
            【统一幽默风格要求（适用于所有神话改写）】
            - 【整体风格定位】：故事整体基调为史诗与庄重，但大约70%的节拍可以通过人物性格中的真实反应产生幽默，其余约30%的关键情节（抉择、牺牲、世界后果、收束画面）基调必须明显偏庄重。幽默用于缓解紧张情绪、放大人物性格，而不是主导故事或变成搞笑段子。目标效果参考：《哪吒之魔童降世》《姜子牙》式的情绪型幽默。
            - 【允许的幽默类型（所有神话通用，必须从中演化，而不是发明新类型）】：
              ...
              4）拆台与错位理解：角色用一句话把前一句"端着"的台词或姿态拆掉，或者基于误解给出跑偏但贴合性格的回应。
              5）夸张比喻与小动作出卖情绪：通过夸张但贴合人物视角的比喻，或一个小动作（挠头、眼神飘开、偷偷躲在某物后面等）出卖真实情绪，形成轻微反差感。
              6）突然认真反转：前一秒还在嘴硬/打岔，下一秒突然认真一句，形成情绪落差，这句话不能是"鸡汤总结"，而是贴着人物经验的具体感受。
            - 【关键情节与普通情节的区分】：
              - 当当前节拍卡的"场景目标"明显属于【关键转折/重大抉择/牺牲代价/世界后果/终极使命完成/收束画面】这一类时，整体基调必须庄重凝重，可以完全不使用幽默，或仅保留最多1句不削弱庄重感的轻描淡写式回应。
              - 当"场景目标"属于铺垫、日常互动、情绪累积、尝试行动、寻找突破口、行动过程中的间隙等非关键情节时，可以自然加入轻度幽默，让整体观感更轻松。
            - 【幽默密度与载体要求（必须严格遵守）】：（重点：把"对话里的幽默"作为第一承载通道，避免长篇自言自语）
              - 篇幅目标：整篇控制在约1500-2500字时，总的幽默出现次数目标为【10-16处】，三幕平均分布，每幕建议【3-6处】；若总字数略有浮动，可在对应比例范围内调整笑点次数。
            - 单次幽默长度：单个笑点以1句话为基准，但允许出现【连续2句的"一抛一接"】对白或内心吐槽，形成更清晰的包袱和回应，不要长段堆砌；**严禁出现一大段没有对话、全靠主角自言自语撑起幽默的长段落**。
            - 三通道承载：幽默**以对白为主**，旁白/内心与动作镜头作为辅助来源：
              * 【对白笑点（第一优先级）】：角色之间的抛接、互怼、轻微挖苦、打趣调侃、错位理解，是主要的幽默承载方式。
              * 【旁白/内心笑点（辅助）】："正经叙述语气 + 很人间的结果"式冷面吐槽或具体心理感受，用来给对白增加一层反差或补刀。
              * 【动作/镜头笑点（辅助）】：小动作出卖情绪、物件反差（比如用不合时宜的东西凑合着用）、"嘴上硬身体很诚实"的反差，用来配合对话形成画面感。
            - 每张节拍卡对应的小节中，默认应出现【至少2个笑点】：其中至少【1个】必须是【对白里的笑点】（互怼、调侃、轻微讽刺、打趣），另1个可以是继续通过对白完成，也可以来自旁白/内心或动作反差；若该节为明显庄重的关键情节，可酌情降低到1个极克制的小笑点，或完全关闭幽默。
            - 【对白与独白硬约束】：
              - 每个节拍卡对应的小节中，**必须至少出现1轮完整对话**（A说一句 + B接一句，算一轮），可以是主角和副角、两个副角之间，或者主角与环境“回声式”对话。
              - 主角连续自言自语（单人说话、没有他人插话）**不允许超过2句**；一旦出现第三句，必须被其他角色打断、接话或转换成对白形式，否则视为未按要求执行。
              - 世界观背景、灾情信息、规则解释等优先通过【副角对白或冲突】抛出，**禁止整段全靠主角长篇独白解释背景**。
            - 【必须参考样本集】：必须参考知识库中"神话重写·哪吒风格（幽默+亲子）"的样本，学习其幽默表达方式和处理方式。样本特点包括：生活化比喻、自嘲式吐槽、在严肃场景中加入轻松元素、对白/旁白/动作三通道分布笑点。
            - 【禁止内容】：
              - 现代社会或职业隐喻（如老板、打工、公司、加班、工资单等）；
              - 网络流行语或口语化表达（如妈耶、绝了、离谱、我人麻了、躺平等）；
              - 为搞笑而刻意设计的低俗段子、身体羞辱或恶趣味笑料；
              - 任何破坏神话世界观或时代感的表达。
              - 把幽默建立在对【弱者/无助者】的羞辱、群体围观取乐、下药或捆绑、长期折磨、身体羞辱等"虐人取乐"桥段之上（例如：为了让读者发笑而设计围殴、戏弄、绑住某个角色的情节）。
            - 【要求】：幽默必须来源于人物当下的真实情绪或处境，应短而自然，每次1-2句话即可；绝对禁止通过羞辱、恶作剧、下药或肢体控制来制造笑点，优先通过对白/内心吐槽/错位理解/动作反差来承载幽默。
            ...
            - 【副角功能位模板（推荐至少启用3个）】：
              - 拆台嘴碎位（喜剧引擎）：负责在主角说出“史诗台词”或严肃宣言时，用生活化、接地气的问题或吐槽拆台，形成互怼效果，同时抛出现实难题（水不够喝、路走错、东西丢了等）。
...
def split_outline_to_acts(overall_outline, theme, rag_content):
    ...
    system_message = {
        ...
        "content": get_myth_system_prompt_base(rag_content) + f"""
            ...
            - 在设计每张【镜头节拍卡】时，要为后续的幽默留出空间：预留至少一个【对白上的抛接点】和一个【非对白的反差点】（可通过画面要素或情绪推动中的具体细节体现），方便在实际写作时从对白、旁白、动作三条通道里自然长出笑点。
...
4. 幽默强度与通道要求：
   - 强度分级：可以隐含地将笑点分为三档——【1级：轻轻一抹（微笑/轻嘟囔）】【2级：明显好笑（反差+拆台）】【3级：爆点（误会升级/连锁反应/一抛一接带来的小高潮）】。
   - 幕别倾向：第一幕以 1-2 级为主（偏暖场）、第二幕允许更多 2 级并在个别非关键节拍中出现 1-2 个 3 级爆点、第三幕以 1-2 级为主（收束但不断气）。
   - 若当前节拍卡的"场景目标"属于铺垫、日常互动、情绪累积、尝试行动、寻找突破口、行动过程中的间隙等【非关键情节】，本小节中【应出现至少 2 个笑点】，其中至少 1 个必须是【非对白笑点】（旁白吐槽、内心嘟囔、动作/镜头反差等），另一个可以是对白笑点或再次使用非对白笑点，整体以 2 级强度为主，偶尔允许出现 3 级小爆点。
   - 若当前节拍卡的"场景目标"属于【关键转折/重大抉择/牺牲代价/世界后果/终极使命完成/收束画面】，整体基调必须庄重，可以将笑点压缩为 0-1 个极克制的 1级小笑点（多为轻描淡写式回应或内心一句话），严禁削弱牺牲感。
   - 单个笑点以 1 句话为主，允许出现【连续 2 句"一抛一接"】的结构（例如一人抛梗，另一人/旁白接梗），不要写成长段密集抖机灵。
   - 幽默类型仅限：压力下的嘴硬、自嘲式幽默、史诗场景里的生活化感受、拆台与错位理解、夸张比喻与小动作出卖情绪、突然认真反转；严禁使用现代职场/网络流行语、低俗/恶趣/侮辱性桥段。
