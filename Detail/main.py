...
def get_myth_system_prompt_base(reference_content=None):
    ...
    return f"""
            ...
            - 【幽默密度与载体要求（必须严格遵守）】：
              ...
              - 单次幽默长度：单个笑点以1句话为基准，但允许出现【连续2句的"一抛一接"】对白或内心吐槽，形成更清晰的包袱和回应，不要长段堆砌。
              - 三通道承载：幽默必须分散在【对白 + 旁白/内心 + 动作镜头】三种通道中，严禁全部集中在对白里：
                * 对白笑点：角色之间的抛接、拆台、误解。
                * 旁白/内心笑点："正经叙述语气 + 很人间的结果"式冷面吐槽或具体心理感受。
                * 动作/镜头笑点：小动作出卖情绪、物件反差（比如用不合时宜的东西凑合着用）、"嘴上硬身体很诚实"的反差。
              - 每张节拍卡对应的小节中，默认应出现【至少2个笑点】：其中至少1个必须是【非对白笑点】（旁白吐槽、内心嘟囔、动作/镜头反差等），另一个可以是对白或再次加一个非对白笑点；若该节为明显庄重的关键情节，可酌情降低到1个极克制的小笑点，或完全关闭幽默。
